...
export class CatalogSearch extends CommonMethods {
  ...
  infoAboutAddedIntoBasket: string =
  ...
  selectFilter(categoryName: string) {
    cy.get(this.items).contains(categoryName).click({ force: true });
  }

  checkIfProductIsOnItemList(productName: string) {
    cy.get(this.itemProducts)
  ...
  checkIfProductNameFromBasketIsTheSameAsAdded() {
    cy.get(this.pageTitleWrapper)
      .invoke("text")
      .then((productName) => {
        cy.get(this.miniCart).click();
        cy.get(this.productItemName)
          .invoke("text")
          .then((productNameFromMiniCart) => {
